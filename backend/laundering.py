...
def fetch_past_transactions(timestamp, days=3):
    ...
    """
    args = (past_time, timestamp)
    past_transactions = get_db(query, args)
    return pd.DataFrame(past_transactions, columns=["sender_account", "receiver_account", "amount", "timestamp"])
...
def compute_fraud_features(df):
    ...
    for idx, row in df.iterrows():
        past_data = fetch_past_transactions(row['timestamp'])
...
def train_model():
    historical_data = pd.read_csv("historical_transactions.csv", dtype={"sender_account": str, "receiver_account": str})
...
def predict_fraud(new_transaction):
    ...
    query = """
        INSERT INTO fraud_predictions (sender_account, receiver_account, amount, timestamp, is_fraud, fraud_reason)
        VALUES (%s, %s, %s, %s, %s, %s)
    """
    args = (new_transaction['sender_account'], new_transaction['receiver_account'], new_transaction['amount'], new_transaction['timestamp'], fraud_prediction, fraud_reason)
    put_db(query, args)
...
fraud_prediction, fraud_reason = predict_fraud(new_transaction)
